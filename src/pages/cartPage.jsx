            ...
            <main className="mx-auto max-w-7xl px-5 py-12 lg:px-8">
                ...
                {!isAuthenticated ? (
                    <div className="flex min-h-[55vh] flex-col items-center justify-center text-center">
                        ...
                        <h2 className="mt-5 text-3xl font-black text-slate-950">Sign in to view your cart</h2>
                        <p className="mt-3 text-slate-500">Your cart items will be available after you log in.</p>
                        <Link to="/login" className="mt-6 rounded-xl bg-slate-950 px-5 py-3 font-bold text-white transition hover:bg-sky-600">Log in</Link>
                    </div>
                ) : cart.length === 0 ? (
                    <div className="flex min-h-[55vh] flex-col items-center justify-center text-center">
                        <FaCartShopping className="text-5xl text-sky-500" />
                    ...
                    <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_360px]">
                        ...
                        <aside className="h-fit rounded-2xl bg-slate-950 p-6 text-white shadow-xl"><h2 className="text-xl font-bold">Order summary</h2><div className="mt-6 flex justify-between border-b border-white/15 pb-4 text-slate-300"><span>Subtotal</span><span>${subtotal.toFixed(2)}</span></div><div className="mt-4 flex justify-between text-lg font-black"><span>Total</span><span>${subtotal.toFixed(2)}</span></div><button type="button" onClick={() => navigate("/checkout")} className="mt-7 w-full rounded-xl bg-amber-400 px-5 py-3 font-black text-slate-950 transition hover:bg-amber-300">Checkout</button><p className="mt-5 flex items-center justify-center gap-2 text-center text-xs text-slate-400"><FaShieldHalved /> Secure checkout experience</p></aside>
                    </div>
                )}
            </main>
        </div>
